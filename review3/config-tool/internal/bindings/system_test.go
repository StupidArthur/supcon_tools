...
func sampleExportRows() []map[string]any {
	return []map[string]any{
		{"_cycle": 0, "_sim_time": 1000.0, "_need_sample": true, "value": 1.25},
		{"_cycle": 1, "_sim_time": 1000.5, "_need_sample": true, "value": 2.5},
	}
}
...
func TestParseBatchCell(t *testing.T) {
	tests := []struct {
		...
		{name: "need sample true", header: "_need_sample", value: "True", want: true},
		{name: "need sample false", header: "_need_sample", value: "false", want: false},
		{name: "need sample one", header: "_need_sample", value: "1", want: true},
		{name: "need sample zero", header: "_need_sample", value: "0", want: false},
		{name: "need sample invalid", header: "_need_sample", value: "", wantErr: true},
		{name: "sim time", header: "_sim_time", value: "1000.5", want: 1000.5},
		{name: "sim time invalid", header: "_sim_time", value: "abc", wantErr: true},
		{name: "sim time nan", header: "_sim_time", value: "NaN", wantErr: true},
		{name: "numeric signal", header: "pid2.PV", value: "0.8", want: 0.8},
		{name: "text signal", header: "state", value: "AUTO", want: "AUTO"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := parseBatchCell(tt.header, tt.value)
			if tt.wantErr {
				if err == nil {
...
func TestParseCSVHiddenFields(t *testing.T) {
	...
	content := "_sim_time,_need_sample,pid2.PV\n1000.5,True,0.8\n1001.0,False,0.9\n"
	...
	wantCols := []string{"_cycle", "_sim_time", "_need_sample", "pid2.PV"}
	if !equalStrings(result.Columns, wantCols) {
		t.Fatalf("columns got %v, want %v", result.Columns, wantCols)
	}
	...
	row0 := result.Rows[0]
	if row0["_cycle"] != 0 {
		t.Fatalf("row0 _cycle = %v, want 0", row0["_cycle"])
	}
	if row0["_sim_time"] != float64(1000.5) {
		t.Fatalf("row0 _sim_time = %v, want 1000.5", row0["_sim_time"])
	}
	if row0["_need_sample"] != true {
		t.Fatalf("row0 _need_sample = %v, want true", row0["_need_sample"])
	}
	if row0["pid2.PV"] != float64(0.8) {
		t.Fatalf("row0 pid2.PV = %v, want 0.8", row0["pid2.PV"])
	}

	row1 := result.Rows[1]
	if row1["_cycle"] != 1 {
		t.Fatalf("row1 _cycle = %v, want 1", row1["_cycle"])
	}
	if row1["_need_sample"] != false {
		t.Fatalf("row1 _need_sample = %v, want false", row1["_need_sample"])
	}
}

func TestParseCSVInvalidHiddenFields(t *testing.T) {
	dir := t.TempDir()
	...
	cases := []struct {
		...
		{name: "empty sim_time", content: "_sim_time,_need_sample,pid2.PV\n,True,0.8\n"},
		{name: "non-numeric sim_time", content: "_sim_time,_need_sample,pid2.PV\nabc,True,0.8\n"},
		{name: "invalid need_sample", content: "_sim_time,_need_sample,pid2.PV\n1000.5,maybe,0.8\n"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			csvPath := filepath.Join(dir, tc.name+".csv")
			if err := os.WriteFile(csvPath, []byte(tc.content), 0o644); err != nil {
				t.Fatal(err)
			}
			_, err := parseCSV(csvPath)
			if err == nil {
				t.Fatal("expected error for invalid hidden field")
			}
		})
	}
}
...
func TestBuildConvertExportArgs(t *testing.T) {
	...
	if !equalStrings(got, want) {
		t.Fatalf("xlsx args got %v, want %v", got, want)
	}
	...
	if !equalStrings(got2, want2) {
		t.Fatalf("csv args got %v, want %v", got2, want2)
	}
	for _, arg := range got2 {
		if arg == "--sheet-name" {
			t.Fatal("csv args must not contain --sheet-name")
		}
	}
}
...
func TestExportRowsFormattedUnifiedCommand(t *testing.T) {
	...
	b.setCommandFactory(func(name string, arg ...string) *exec.Cmd {
		capturedArgs = arg
		if err := os.WriteFile(outCSV, []byte("timeStamp,value\n时间戳,某工业数据\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		cmd := exec.Command("cmd", "/C", "exit", "0")
		return cmd
	})
	err = b.ExportRowsFormatted([]string{"value"}, rows, outCSV, "csv", "")
	if err != nil {
		t.Fatalf("csv export: %v", err)
	}
	if !containsArg(capturedArgs, "--convert-export") {
		t.Fatalf("csv must use --convert-export, got args: %v", capturedArgs)
	}
	if !containsArgPair(capturedArgs, "--template", "prediction") {
		t.Fatalf("csv must pass --template prediction, got args: %v", capturedArgs)
	}
	if containsArg(capturedArgs, "--sheet-name") {
		t.Fatalf("csv must not pass --sheet-name, got args: %v", capturedArgs)
	}

	outXLSX := filepath.Join(dir, "out.xlsx")
	b.setCommandFactory(func(name string, arg ...string) *exec.Cmd {
		capturedArgs = arg
		if err := os.WriteFile(outXLSX, []byte("PK\x03\x04dummy"), 0o644); err != nil {
			t.Fatal(err)
		}
		cmd := exec.Command("cmd", "/C", "exit", "0")
		return cmd
	})
	err = b.ExportRowsFormatted([]string{"value"}, rows, outXLSX, "xlsx", "控制器")
	if err != nil {
		t.Fatalf("xlsx export: %v", err)
	}
	if !containsArg(capturedArgs, "--convert-export") {
		t.Fatalf("xlsx must use --convert-export, got args: %v", capturedArgs)
	}
	if !containsArgPair(capturedArgs, "--template", "prediction") {
		t.Fatalf("xlsx must pass --template prediction, got args: %v", capturedArgs)
	}
	if !containsArgPair(capturedArgs, "--sheet-name", "控制器") {
		t.Fatalf("xlsx must pass --sheet-name 控制器, got args: %v", capturedArgs)
	}
}
