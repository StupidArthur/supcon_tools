...
class EngineDiagnosticProvider(DiagnosticProvider):
    ...
    def __init__(self, engine: UnifiedEngine, redis_client=None):
        """
        初始化 Engine 诊断提供者

        Args:
            engine: UnifiedEngine 实例
            redis_client: Redis 客户端实例（可选；standalone 模式下传 None 即可，
                         诊断数据只在内存中收集，不推送到 Redis）
        """
    ...
    def collect_diagnostics(self) -> List[DiagnosticItem]:
        ...
        try:
            ...
            # Redis 连接状态：standalone 模式已禁用 realtime_publisher，
            # 永远标记为 disabled，不再做 ping 检查或异常捕获
            items.append(DiagnosticItem(
                name="redis_connection_status",
                value="disabled",
                unit="",
                description="Redis连接状态（standalone 模式已禁用）",
                data_type="string"
            ))
