...
def run_performance(config_path: Path, cycle_counts: list[int]) -> None:
    """
    测试不同周期数下的性能

    注意：函数名以 ``run_`` 开头（而非 ``test_``），不会被 pytest 收集为测试用例；
    本文件实质是一个 benchmark 脚本，通过 ``python tests/test_generator_performance.py`` 直接运行。
...
def main():
    ...
    run_performance(config_path, cycle_counts)
