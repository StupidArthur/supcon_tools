...
$outDir = Join-Path $RepoRoot "output\automation_stage1_$stamp"
...
try {
    ...
    Run-Step "doctor" {
        & $PythonExe -m ua_test_harness.doctor `
            --base-url $BaseUrl `
            --username $Username `
            --local-ip $LocalIp `
            --output (Join-Path $outDir "doctor.json")
    }
    ...
    } | ConvertTo-Json -Depth 8 | Set-Content -Encoding UTF8 (Join-Path $outDir "stage1-result.json")
