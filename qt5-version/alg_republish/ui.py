...
class MainWindow(QtWidgets.QWidget):
    ...
    def _init_ui(self):
        self.setWindowTitle("算法重发布工具_低系统版本  |  designed by @yuzechao")
        ...
        self.console_emitter.signal.connect(
            self._append_log, QtCore.Qt.QueuedConnection)
        ...
        def do_fetch():
            try:
                ...
                QtCore.QMetaObject.invokeMethod(
                    self.algo_list_widget, "setPlainText", QtCore.Qt.QueuedConnection,
                    QtCore.Q_ARG(str, "\n".join(lines) if lines else "  无已发布的算法"))

                if released:
                    self._log("[提示] 点击「执行发布流程」开始逐个重新发布")
                    QtCore.QMetaObject.invokeMethod(
                        self.exec_btn, "setEnabled", QtCore.Qt.QueuedConnection,
                ...
                QtCore.QMetaObject.invokeMethod(
                    self.view_btn, "setEnabled", QtCore.Qt.QueuedConnection,
                    QtCore.Q_ARG(bool, True))
