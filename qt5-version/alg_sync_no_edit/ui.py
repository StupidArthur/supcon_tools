...
class MainWindow(QtWidgets.QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle("算法同步工具_不编辑_低系统版本  |  designed by @yuzechao")
        ...
        self.console_emitter.signal.connect(
            self.append_console, QtCore.Qt.QueuedConnection)
        self.confirm_ready.connect(
            self._doShowConfirmDialog, QtCore.Qt.QueuedConnection)
        self.fail_ready.connect(
            self._doShowFailDialog, QtCore.Qt.QueuedConnection)
    ...
    def on_start(self):
        ...
        self.log("=" * 60)
        self.log("同步本地算法文件与平台发布状态（不上传编辑）")
        self.log("=" * 60)
        ...
        def do_connect():
            try:
                ...
                QtCore.QMetaObject.invokeMethod(
                    self.start_btn, "setEnabled", QtCore.Qt.QueuedConnection,
    ...
    def on_export(self):
        ...
        def do_export():
            try:
                ...
                auth_msg = "（可能是登录已过期，请重新登录）" if getattr(e, "is_auth_error", False) else ""
                self.log(f"[导出失败] {e} {auth_msg}")
                self.log(traceback.format_exc())
            finally:
                QtCore.QMetaObject.invokeMethod(
                    self.export_btn, "setEnabled", QtCore.Qt.QueuedConnection,
                    QtCore.Q_ARG(bool, True))
                QtCore.QMetaObject.invokeMethod(
                    self.start_btn, "setEnabled", QtCore.Qt.QueuedConnection,
                    QtCore.Q_ARG(bool, True))
    ...
    def _doShowConfirmDialog(self):
        try:
            ...
            text_browser.setHtml(
                ...
                f"<b>每个算法将按序执行（不编辑）：</b><br>"
                f"取消发布 → 上传文件 → 重新发布（已发布算法）<br>"
                f"上传文件（未发布算法）<hr>"
                f"<b>确认是否开始更新？</b>"
            )
            ...
            yes_btn = btn_box.addButton("确定", QtWidgets.QDialogButtonBox.AcceptRole)
            no_btn = btn_box.addButton("取消", QtWidgets.QDialogButtonBox.RejectRole)
