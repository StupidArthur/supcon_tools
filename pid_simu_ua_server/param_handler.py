...
class ParamHandler:
    ...
    def get_tank_params(tank_height: QLineEdit, tank_radius: QLineEdit,
        ...
        """获取水箱参数"""
        height = ParamHandler.get_float_value(tank_height, "2.0")
        if height <= 0:
            raise ValueError("水箱高度必须大于0")
        
        radius = ParamHandler.get_float_value(tank_radius, "0.5")
        if radius <= 0:
            raise ValueError("水箱半径必须大于0")
        
        inlet_area = ParamHandler.get_float_value(tank_inlet_area, "0.06")
        if inlet_area <= 0:
            raise ValueError("入水口面积必须大于0")
        
        inlet_velocity = ParamHandler.get_float_value(tank_inlet_velocity, "3.0")
        if inlet_velocity < 0:
            raise ValueError("入水速度不能为负数")
        
        outlet_area = ParamHandler.get_float_value(tank_outlet_area, "0.001")
        if outlet_area <= 0:
            raise ValueError("出水口面积必须大于0")
        
        initial_level = ParamHandler.get_float_value(tank_initial_level, "0.0")
        if initial_level < 0:
    ...
    def get_valve_params(valve_min_opening: QLineEdit, valve_max_opening: QLineEdit,
        ...
        """获取阀门参数"""
        min_opening = ParamHandler.get_float_value(valve_min_opening, "0.0")
        max_opening = ParamHandler.get_float_value(valve_max_opening, "100.0")
        ...
        full_travel_time = ParamHandler.get_float_value(valve_full_travel_time, "5.0")
    ...
    def get_pid_params(pid_kp: QLineEdit, pid_ti: QLineEdit, pid_td: QLineEdit,
        ...
        """获取PID参数"""
        kp = ParamHandler.get_float_value(pid_kp, "12.0")
        if kp < 0:
            raise ValueError("比例系数不能为负数")
        
        ti = ParamHandler.get_float_value(pid_ti, "30.0")
        if ti < 0:
            raise ValueError("积分时间不能为负数")
        
        td = ParamHandler.get_float_value(pid_td, "0.15")
        if td < 0:
            raise ValueError("微分时间不能为负数")
        
        h = ParamHandler.get_float_value(pid_h, "100.0")
        l = ParamHandler.get_float_value(pid_l, "0.0")
        if h <= l:
        ...
        return {
            ...
            'pv': ParamHandler.get_float_value(pid_pv, "0.0"),
            'mv': ParamHandler.get_float_value(pid_mv, "0.0"),
            'h': h,
            'l': l
        }
    
    @staticmethod
    def get_sv_values(pid_sv: QLineEdit, parent_widget=None) -> List[float]:
        """获取SV设定值列表（逗号分隔）"""
        ...
        try:
            ...
            if parent_widget:
                QMessageBox.warning(parent_widget, "警告", "SV设定值格式错误，请使用逗号分隔的数字，如：0,1.5,0")
            return [0.0]
